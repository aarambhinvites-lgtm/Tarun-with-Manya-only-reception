export const weddingData = {
  couple: {
    groom: "Tarun",
    bride: "Manya",
    initials: "T & M"
  },

  // Target date for countdown: 11 December 2026
  weddingDate: "2026-12-11T00:00:00",

  storyChapters: [
    {
      heading: "It Started in Seattle…",
      text: [
        "In the summer of 2022, a first date in Seattle became the start of something neither of us expected."
      ]
    },
    {
      heading: "Some Stories Find Their Way Back",
      text: [
        "Life took us to different cities and different chapters, but never completely out of each other’s lives.",
        "When the timing finally caught up with us, we found our way back, and this time, we knew."
      ]
    },
    {
      heading: "From Seattle to Vancouver",
      text: [
        "A few adventures later, our story brought us to Vancouver, with one unforgettable stop in Cabo along the way.",
        "Sunshine. Ocean. Cabo.",
        "And one very important question.",
        "Tarun asked. Manya said yes. 💍"
      ]
    }
  ],

  families: {
    groom: {
      parents: [
        "Sushil Bedi",
        "Dr. (Col.) Harsh Kumar Bedi"
      ],
      siblings: [
        { relation: "Sister & Brother-in-law", name: "Nisha Bedi & Varun Pahwa" }
      ]
    },
    bride: {
      parents: [
        "Anjana Dadarya",
        "Anil Kumar Dadarya"
      ],
      siblings: [
        { relation: "Sister", name: "Divya Dadarya" }
      ]
    }
  },

  events: [
    {
      id: "haldi-sangeet",
      title: "Carnival of Love",
      subheading: "Phoolon Ki Haldi aur Sangeet",
      date: "10 December 2026",
      day: "Thursday",
      time: "12:30 PM",
      venue: "Regenta Central Klassik, Ludhiana",
      locationUrl: "https://www.google.com/maps/place/Regenta+Central+Klassik/@30.892329,75.847686,17z/data=!3m1!4b1!4m9!3m8!1s0x391a83ac9b5502e1:0xaafaff64d33c6778!5m2!4m1!1i2!8m2!3d30.892329!4d75.847686!16s%2Fg%2F1tcztb95?hl=en-us&entry=ttu&g_ep=EgoyMDI2MDkwMi4wIKXMDSoASAFQAw%3D%3D",
      image: "https://res.cloudinary.com/kvup9rzt/image/upload/v1789021472/ChatGPT_Image_Sep_10_2026_11_50_44_AM.webp",
      video: "https://res.cloudinary.com/kvup9rzt/video/upload/v1788699389/Your_paragraph_text_2__1.mov"
    },
    {
      id: "anand-karaj",
      title: "Anand Karaj",
      date: "11 December 2026",
      day: "Friday",
      time: "Morning — Time to be announced",
      venue: "Gurdwara Sri Guru Singh Sabha, Sarabha Nagar, Ludhiana",
      locationUrl: "https://maps.google.com?q=D-Block,%20Block%20D,%20Sarabha%20Nagar,%20Ludhiana,%20Punjab%20141001&ftid=0x391a83cd9cc9d721:0xcb058d24622ed52&entry=gps&shh=CAE&lucs=,100834231,121816459,94297699,94231188,94280568,47071704,94218641,94282134,100813469,94286869,100820247,100822499&g_st=ic",
      video: "https://res.cloudinary.com/kvup9rzt/video/upload/v1788699375/Your_paragraph_text_2.mov"
    },
    {
      id: "reception",
      title: "Reception",
      date: "11 December 2026",
      day: "Friday",
      time: "Evening — Time to be announced",
      venue: "Ambrosia Paradise, Ludhiana - Malerkotla Road",
      locationUrl: "https://maps.google.com?q=Ambrosia%20Paradise%20%7C%20Wedding%20Resort%20%7C%20All%20Weather%20Lawn,%20Ludhiana%20-%20Malerkotla%20Rd,%20VPO,%20Kaind,%20Punjab%20141116,%20India&ftid=0x391079b319cbf909:0x6fc9faee72016073&entry=gps&shh=CAE&lucs=,100834231,121816459,94297699,94231188,94280568,47071704,94218641,94282134,100813469,94286869,100820247,100822499&g_st=ic",
      video: "https://res.cloudinary.com/kvup9rzt/image/upload/v1788774868/ChatGPT_Image_Sep_7_2026_03_24_16_PM.png"
    }
  ],

  assets: {
    heroVideo: "https://res.cloudinary.com/kvup9rzt/video/upload/v1789378630/MT_Initial_Slide.mp4",
    heroPoster: "https://res.cloudinary.com/kvup9rzt/video/upload/so_0/v1789378630/MT_Initial_Slide.jpg",
    heroBg: "https://res.cloudinary.com/kvup9rzt/image/upload/v1788415384/WhatsApp_Image_2026-08-25_at_7.43.29_AM_3.jpg",
    music: "https://res.cloudinary.com/kvup9rzt/video/upload/v1789134649/Jashn_E_Bahaaraa_Instrumental_Flute_Jodhaa_Akbar_128_Kbps.mp3",
    monogram: "https://res.cloudinary.com/kvup9rzt/image/upload/e_trim/v1788890221/tm_logo.png",
    invitationBg: "https://res.cloudinary.com/kvup9rzt/image/upload/v1787772800/Tarun_Manya_Wedding_Monogram_1.png",
    logo: "/assets/branding/logo.png",
    sticker: "/assets/branding/sticker.png",
    proposalVideoUrl: "https://www.w3schools.com/html/mov_bbb.mp4",
    ogImage: "https://res.cloudinary.com/kvup9rzt/image/upload/v1789449673/ChatGPT_Image_Sep_14_2026_11_30_18_PM.webp",
    favicon: "https://res.cloudinary.com/kvup9rzt/image/upload/v1788890221/tm_logo.png"
  },

  social: {
    instagram: "https://instagram.com/aarambhinvites"
  }
};
